    ...
    <tr className="bg-gray-50/70 dark:bg-gray-900/40 border-b border-gray-100 dark:border-gray-700">
      <td colSpan={7} className="px-6 py-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1 text-xs text-gray-600 dark:text-gray-400">
          {vendor.sector && (
            <span>
              <span className="font-medium text-gray-700 dark:text-gray-300">Sector:</span> {vendor.sector}
            </span>
          )}
          {vendor.location && (
            <span>
              <span className="font-medium text-gray-700 dark:text-gray-300">Location:</span> {vendor.location}
            </span>
          )}
          {vendor.serviceType && (
            <span>
              <span className="font-medium text-gray-700 dark:text-gray-300">Service:</span> {vendor.serviceType}
            </span>
          )}
          {vendor.populationImpacted && (
            <span>
              <span className="font-medium text-gray-700 dark:text-gray-300">Population:</span> {vendor.populationImpacted}
            </span>
          )}
          {vendor.dataTypes && vendor.dataTypes.length > 0 && (
            <span>
              <span className="font-medium text-gray-700 dark:text-gray-300">Data:</span> {vendor.dataTypes.join(', ')}
            </span>
          )}
          {vendor.contact && (
            <span>
              <span className="font-medium text-gray-700 dark:text-gray-300">Contact:</span> {vendor.contact}
            </span>
          )}
          {hints.length > 0 && (
...
export interface PortfolioViraRegisterProps {
  vendors: VendorRadar[];
  stats: PortfolioStats;
}

/** VIRA-style portfolio register (sortable table, recommendations, print hint) — used on the unified portfolio reports page. */
const PortfolioViraRegister: React.FC<PortfolioViraRegisterProps> = ({ vendors, stats }) => {
    ...
    return vendors
      ...
      .sort((a, b) => {
        ...
        if (sortKey === 'name') cmp = a.name.localeCompare(b.name);
          ...
          cmp = suggestedNextReviewIsoDate(a.inherentRisk).localeCompare(suggestedNextReviewIsoDate(b.inherentRisk));
        }
  ...
  return (
    <>
                      ...
                      <React.Fragment key={vendor.id}>
                        ...
                        >
                          <td className="px-4 py-3">
                            ...
                            {vendor.sector && <div className="text-xs text-gray-400">{vendor.sector}</div>}
                          </td>
                          <td className="px-4 py-3 text-gray-600 dark:text-gray-300 capitalize">{vendor.category}</td>
                          <td className="px-4 py-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200/70 bg-gray-50 px-4 py-3 text-sm text-gray-500 dark:border-gray-800 dark:bg-gray-900/50 dark:text-gray-400">
        <span>
          Use <strong className="text-gray-700 dark:text-gray-300">Open summary report</strong> in Reports &amp; exports
          above, then choose <strong className="text-gray-700 dark:text-gray-300">Print / Save as PDF</strong> for a
...
export default PortfolioViraRegister;
